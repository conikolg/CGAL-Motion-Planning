...
typename Arrangement::Halfedge_const_handle
add_vertical_segment(Arrangement &arr, typename Arrangement::Vertex_handle v, CGAL::Object obj, Kernel &ker) {
    ...
    if (CGAL::assign(vh, obj)) {
        ...
        if (cmp_x(v->point(), hh->target()->point()) == CGAL::EQUAL) {
            ...
            seg = Segment(v->point(), hh->target()->point());
            ...
            Line vert_line(v->point(), Point(v->point().x(), v->point().y() + 1));
            typename Arrangement::Point_2 point;
            CGAL::assign(point, ker.intersect_2_object()(supp_line, vert_line));
            seg = Segment(v->point(), point);
            arr.split_edge(arr.non_const_handle(hh),
                           Segment(hh->source()->point(), point),
                           Segment(point, hh->target()->point()));
            v2 = arr.non_const_handle(hh->target());
        }
    }
...
void vertical_decomposition(Arrangement &arr, Kernel &ker) {
    typedef std::pair<typename Arrangement::Vertex_const_handle, std::pair<CGAL::Object, CGAL::Object> > Vd_entry;
